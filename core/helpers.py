...
def ascii2d_keyboard(file_path: str):
    ...
    [
        InlineKeyboardButton(text="✅", callback_data=SearchCallbackData(action='start_ascii2d_search', file_path=file_path).pack()), 
        InlineKeyboardButton(text="❌", callback_data=SearchCallbackData(action='cancel_ascii2d_search', file_path=file_path).pack())
    ]
])
