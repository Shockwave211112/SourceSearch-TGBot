...
bot_id = os.environ.get('TG_ID')
bot = telebot.TeleBot(bot_id)
sauce_api = SauceNao(os.environ.get('SAUCE_ID'))
...
def find(message):
  if message.content_type == 'photo':
    try:
      ...
      results = sauce_api.from_url("https://api.telegram.org/file/bot" +
                                   bot_id + "/" + file_info.file_path)
      number_of_result = 0
          ...
          if len(sauce.urls) != 0:
            ...
            if 'source' in sauce.raw['data']:
              if urlValidator(sauce.raw['data']['source']):
                ...
                url_tmp = types.InlineKeyboardButton(
                  name.hostname, url=sauce.raw['data']['source'])
                markup.add(url_tmp)
...
def start(message, res=False):
  bot.send_message(message.chat.id,
                   'Пришли картинку, я попробую найти оригинал и автора')
